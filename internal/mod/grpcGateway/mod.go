...
func (m *Mod) PreInit(hub *kernel.Hub) error {
	grpc_zap.ReplaceGrpcLoggerV2(logx.NameSpace("grpc").Desugar())
...
// PostInit the grpc-gateway module with a grpc server , it is important to note that the gateway is created here
func (m *Mod) PostInit(hub *kernel.Hub) error {
	...
	http.Any("/api/*any", func(c *gin.Context) {
